...
async def health() -> dict[str, Any]:
    return {
        ...
        "cloud_configured": settings.has_cognee_cloud,
    }
...
# --------------------------------------------------------------------------
# Compliance cockpit + erasure ledger
# --------------------------------------------------------------------------
@app.get("/api/compliance")
async def compliance() -> dict[str, Any]:
    """Per-subject compliance status derived from the audits run so far."""
    subjects = []
    for c in CUSTOMERS:
        a = AUDITS.get(c.id, {})
        baseline, post = a.get("baseline"), a.get("post")
        if post is not None:
            status = "erased_verified" if post["leaks"] == 0 else "erased_incomplete"
        elif baseline is not None:
            status = "audited"
        else:
            status = "in_memory"
        subjects.append({
            "id": c.id, "name": c.name, "city": c.city, "complaint": c.complaint,
            "contamination_before": baseline["contamination_score"] if baseline else None,
            "leaks_before": baseline["leaks"] if baseline else None,
            "leaks_after": post["leaks"] if post else None,
            "residual": (sum(1 for r in post["results"] if r["verdict"] == "LEAK")
                         if post else None),
            "status": status,
        })
...
def _ledger_entries() -> list[dict[str, Any]]:
    ...
    for rec in CERTS.values():
        ...
        items.append({
            ...
            "data_subject": cert["data_subject"],
            "data_subject_name": cert["data_subject_name"],
            "executed_at": cert["executed_at"],
            "erasure_mode": cert["erasure_mode"],
            "leaks_before": v["leaks_before"],
            "leaks_after": v["leaks_after"],
...
async def ledger_export(fmt: str = "json") -> Response:
    entries = _ledger_entries()
    if fmt == "csv":
        import io
        import csv
        buf = io.StringIO()
        cols = ["certificate_id", "data_subject", "data_subject_name", "executed_at",
                "erasure_mode", "leaks_before", "leaks_after", "verdict", "signature"]
        ...
        return Response(
            ...
            headers={"Content-Disposition": "attachment; filename=lethe_erasure_ledger.csv"})
    import json as _json
    body = _json.dumps({"issuer": settings.ISSUER, "entries": entries}, indent=2)
    return Response(
        content=body, media_type="application/json",
        headers={"Content-Disposition": "attachment; filename=lethe_erasure_ledger.json"})


# --------------------------------------------------------------------------
# Real Cognee Cloud knowledge-graph viewer
# --------------------------------------------------------------------------
@app.get("/api/cloud/graph/{customer_id}")
async def cloud_graph(customer_id: str) -> dict[str, Any]:
    """Fetch and normalize the actual cognified graph for a customer dataset
    from Cognee Cloud (GET /api/v1/datasets/{id}/graph)."""
    if not settings.has_cognee_cloud:
        raise HTTPException(400, "Cognee Cloud not configured.")
    raw = await memory.CogneeCloudBackend(
        settings.COGNEE_API_URL, settings.COGNEE_API_KEY, settings.COGNEE_TENANT_ID,
    ).graph(f"customer_{customer_id}")
    return raw


def _cloud_backend() -> "memory.CogneeCloudBackend":
    if not settings.has_cognee_cloud:
        raise HTTPException(400, "Cognee Cloud not configured.")
    return memory.CogneeCloudBackend(
        settings.COGNEE_API_URL, settings.COGNEE_API_KEY, settings.COGNEE_TENANT_ID)


@app.post("/api/cloud/seed/{customer_id}")
async def cloud_seed(customer_id: str) -> dict[str, Any]:
    """Persistently ingest one customer into Cognee Cloud (add + cognify) so its
    real knowledge graph can be viewed. Slow: cognify runs an LLM server-side."""
    c = get_customer(customer_id)
    if c is None:
        raise HTTPException(404, f"unknown customer {customer_id}")
    await _cloud_backend().remember(c.transcript, f"customer_{customer_id}")
    return {"status": "seeded to cognee cloud", "dataset": f"customer_{customer_id}"}


@app.post("/api/cloud/forget/{customer_id}")
async def cloud_forget(customer_id: str) -> dict[str, Any]:
    c = get_customer(customer_id)
    await _cloud_backend().forget(
        f"customer_{customer_id}", subject_name=c.name if c else None, cascade=False)
    return {"status": "forgotten on cognee cloud", "dataset": f"customer_{customer_id}"}
